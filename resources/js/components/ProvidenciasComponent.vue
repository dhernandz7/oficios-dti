<template>
  <div>
    ...
    <div class="shadow-lg p-4 mb-5 bg-white rounded">
      <button v-if="showBotonReservar" v-on:click="reservarAutomaticamente" id="reservar-providencias" class="btn btn-primary mb-3">
      ...
      <div class="table-responsive">
        <table class="table table-hover border" id="datatable">
          <thead class="bg-dark text-white">
            <tr>
              <th>Asignacion id</th>
              <th>Providencia id</th>
              <th>Providencia anio</th>
              <th>Número de providencia</th>
              <th>Reservado por</th>
              <th>Fecha y hora de reservación</th>
              <th>Estado dla providencia</th>
  ...
  export default {
    ...
    methods: {
        ...
        $("#datatable tbody").on("click", "button.asignar", function(e){
          ...
          Swal.fire({
            ...
            if( result.value) {
              ...
              axios.post(`/api/providencias/reservar`, formulario)
              .then(response => {
                Swal.fire({
                ...
                this.mostrarErrores(error, "Error reservar la providencia", "No pudimos asignar la providencia por los siguientes motivos:<br><br>");
              });
            }
          });
        }.bind(this));
      ...
      reservarAutomaticamente() {
        Swal.fire({
          ...
          if( result.value) {
            ...
            formulario.append('tipo_documento_id', 4);
            formulario.append('user_id', localStorage.getItem("id"));
            axios.post("/api/providencias", formulario)
            .then(response => {
              Swal.fire({
                title: "Providencia reservada",
                type: "info",
            ...
            .catch(error => {
              this.mostrarErrores(error, "Error al reservar una providencia", "No pudimos reservar la providencia por los siguientes motivos:<br><br>");
            });
      ...
      subirPdf() {
        ...
        formulario.append("oficio_id", this.data.oficio_id);
        formulario.append("oficio_anio", this.data.oficio_anio);
        axios.post(`api/providencias/${this.data.asignacion_id}/pdf`, formulario).then(response => {
      ...
      actualizarTabla() {
        this.datatable.ajax.reload(() => Swal.fire({
          title: 'Información',
          html: 'La lista de providencia fue actualizada correctamente',
          type: 'success'}), true)
      }
