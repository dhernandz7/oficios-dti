<template>
  <div>
    ...
    <div class="shadow-lg p-4 mb-5 bg-white rounded">
      <button v-on:click="reservarAutomaticamente" id="reservar-providencia" class="btn btn-primary mb-3">
        <i class="fa fa-bell fa-lg mr-2"></i>
        Reservar providencia
      </button>
        ...
        <table class="table table-hover border" id="datatable">
          <thead class="bg-dark text-white">
            <tr>
              ...
              <th>Número de providencia</th>
              <th>Reservado por</th>
              <th>Fecha y hora de reservación</th>
              <th>Estado de la providencia</th>
        ...
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title font-weight-bold" id="adjuntarModalLabel"><i class="fa fa-file-pdf fa-lg mr-2"></i>
              Providencia {{data.oficio_id}}-{{data.oficio_anio}}/AJ/{{data.iniciales}}
            </h5>
      ...
      inicializarTabla() {
        this.datatable = $('#datatable').DataTable({
          ...
          {'render': function(data, type, row) {
            ...
            if(row.name == null && $("#ultimo").val() == "nada") {
              opciones += `
              <button class="asignar dropdown-item"><i class="fa fa-bell mr-2"></i>Reservar providencia</button>
              `;
              $("#ultimo").val(row.id);
            } else if(row.name == localStorage.getItem('nombre') && row.path == null) {
              opciones += `
      ...
      reservar() {
        $("#datatable tbody").on("click", "button.asignar", function(e){
          ...
          Swal.fire({
            ...
            html: `
            ¿Está seguro de reservar una providencia <span class="font-weight-bold">${this.data.oficio_id}-${this.data.oficio_anio}/AJ/${this.misIniciales}</span>?
            `,
            type: 'question',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            confirmButtonText: '<i class="fa fa-bell fa-lg mr-2"></i>Reservar providencia',
            ...
            if( result.value) {
              ...
              .then(response => {
                Swal.fire({
                  ...
                  type: "info",
                  html: `Se reservó correctamente la providencia <span class="font-weight-bold">${this.data.oficio_id}-${this.data.oficio_anio}/AJ/${this.misIniciales}</span>`
                }).then((result) => {
                ...
                this.mostrarErrores(error, "Error reservar el providencia", "No pudimos asignar el providencia por los siguientes motivos:<br><br>");
              });
            }
          });
        }.bind(this));
          ...
          if( result.value) {
            ...
            .then(response => {
              Swal.fire({
                title: "Providencia reservado",
                type: "info",
                html: `Se reservó correctamente la providencia <span class="font-weight-bold">${response.data.oficio_id}-${response.data.oficio_anio}/AJ/${response.data.iniciales}</span>`
              }).then((result) => {
            ...
            .catch(error => {
              this.mostrarErrores(error, "Error al reservar un providencia", "No pudimos reservar el providencia por los siguientes motivos:<br><br>");
            });
      ...
      subirPdf() {
        ...
        axios.post(`api/providencia/${this.data.asignacion_id}/pdf`, formulario).then(response => {
          ...
          Swal.fire({
            ...
            type: 'success',
            html: `
            Se adjuntó correctamente el pdf a la providencia <span class="font-weight-bold">${this.data.oficio_id}-${this.data.oficio_anio}/AJ/${this.data.iniciales}</span>
            `
          }).then((result) => {
            $("#formulario")[0].reset();
            this.showEmbed = false;
