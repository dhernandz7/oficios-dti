<template>
  <div>
    ...
    <h1 class="h3 mb-4 text-gray-800"><i class="fa fa-book mr-2"></i>Dictámenes</h1>
    ...
    methods: {
      inicializarTabla() {
        this.datatable = $('#datatable').DataTable({
          ...
          {'render': function(data, type, row) {
            ...
            if(row.name == null && $("#ultimo").val() == "nada") {
              opciones += `
              <button class="asignar dropdown-item"><i class="fa fa-bell mr-2"></i>Reservar dictámen</button>
              `;
              $("#ultimo").val(row.id);
            } else if(row.name == localStorage.getItem('nombre') && row.path == null) {
              opciones += `
      ...
      reservar() {
        $("#datatable tbody").on("click", "button.asignar", function(e){
          ...
          Swal.fire({
            ...
            html: `
            ¿Está seguro de reservar el dictámen <span class="font-weight-bold">${this.data.oficio_id}-${this.data.oficio_anio}/AJ/${this.misIniciales}</span>?
            `,
            type: 'question',
            ...
            if( result.value) {
              ...
              axios.post(`/api/dictamen/reservar`, formulario)
              .then(response => {
                Swal.fire({
                  title: "Dictámen reservado",
                  type: "info",
                  html: `Se reservó correctamente el dictámen <span class="font-weight-bold">${this.data.oficio_id}-${this.data.oficio_anio}/AJ/${this.misIniciales}</span>`
                }).then((result) => {
                ...
                this.mostrarErrores(error, "Error reservar el dictámen", "No pudimos asignar el dictámen por los siguientes motivos:<br><br>");
              });
            }
          });
        }.bind(this));
      ...
      reservarAutomaticamente() {
        Swal.fire({
          title: 'Reservar dictámen',
          html: `
          ¿Está seguro de reservar un dictámen?
          `,
          type: 'question',
          showCancelButton: true,
          confirmButtonColor: '#3085d6',
          ...
          if( result.value) {
            ...
            formulario.append('tipo_documento_id', 2);
            formulario.append('user_id', localStorage.getItem("id").split("$")[1]);
            axios.post("/api/dictamen", formulario)
            .then(response => {
              Swal.fire({
                title: "Dictámen reservado",
                type: "info",
                html: `Se reservó correctamente el dictámen <span class="font-weight-bold">${response.data.oficio_id}-${response.data.oficio_anio}/AJ/${response.data.iniciales}</span>`
              }).then((result) => {
            ...
            .catch(error => {
              this.mostrarErrores(error, "Error al reservar un dictámen", "No pudimos reservar el dictámen por los siguientes motivos:<br><br>");
            });
      ...
      subirPdf() {
        ...
        formulario.append("oficio_id", this.data.oficio_id);
        formulario.append("oficio_anio", this.data.oficio_anio);
        axios.post(`api/dictamen/${this.data.asignacion_id}/pdf`, formulario).then(response => {
          $("#adjuntarModal").modal("toggle");
          Swal.fire({
            title: 'Pdf cargado',
            type: 'success',
            html: `
            Se adjuntó correctamente el pdf al dictámen <span class="font-weight-bold">${this.data.oficio_id}-${this.data.oficio_anio}/AJ/${this.data.iniciales}</span>
            `
          }).then((result) => {
            $("#formulario")[0].reset();
            this.showEmbed = false;
      ...
      actualizarTabla() {
        this.datatable.ajax.reload(() => Swal.fire({
          title: 'Información',
          html: 'La lista de dictámenes fue actualizada correctamente',
          type: 'success'}))
      }
    }
  }
