      ...
      reservar() {
        $("#datatable tbody").on("click", "button.asignar", function(e){
          ...
          Swal.fire({
            ...
            html: `
            ¿Está seguro de reservar el dictámen <span class="font-weight-bold">DTI-${this.data.oficio_id}-${this.data.oficio_anio}</span>?
            `,
            type: 'question',
            ...
            if( result.value) {
              ...
              .then(response => {
                Swal.fire({
                  ...
                  type: "info",
                  html: `Se reservó correctamente el dictámen <span class="font-weight-bold">DTI-${this.data.oficio_id}-${this.data.oficio_anio}</span>`
                }).then((result) => {
          ...
          if( result.value) {
            ...
            .then(response => {
              Swal.fire({
                title: "Dictámen reservado",
                type: "info",
                html: `Se reservó correctamente el dictámen <span class="font-weight-bold">DTI-${response.data.oficio_id}-${response.data.oficio_anio}</span>`
              }).then((result) => {
      ...
      subirPdf() {
        ...
        axios.post(`api/dictamen/${this.data.asignacion_id}/pdf`, formulario).then(response => {
          ...
          Swal.fire({
            ...
            type: 'success',
            html: `
            Se adjuntó correctamente el pdf al dictámen <span class="font-weight-bold">DTI-${this.data.oficio_id}-${this.data.oficio_anio}</span>
            `
          }).then((result) => {
            $("#formulario")[0].reset();
            this.showEmbed = false;
