        ...
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title font-weight-bold" id="adjuntarModalLabel"><i class="fa fa-file-pdf fa-lg mr-2"></i>
              Oficio {{data.oficio_id}}-{{data.oficio_anio}}/AJ/{{data.iniciales}}
            </h5>
    ...
    methods: {
      ...
      reservarAutomaticamente() {
        Swal.fire({
          title: 'Reservar oficio',
          html: `
          ¿Está seguro de reservar un oficio?
          `,
          type: 'question',
          showCancelButton: true,
          confirmButtonColor: '#3085d6',
          ...
          if( result.value) {
            ...
            .catch(error => {
              this.mostrarErrores(error, "Error al reservar un oficio", "No pudimos reservar el oficio por los siguientes motivos:<br><br>");
            });
      ...
      subirPdf() {
        ...
        formulario.append("oficio_id", this.data.oficio_id);
        formulario.append("oficio_anio", this.data.oficio_anio);
        axios.post(`api/oficio/${this.data.asignacion_id}/pdf`, formulario).then(response => {
